...
export default function DashBoardPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1280px] pb-40 overflow-hidden">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            {[
              ...
              <motion.div key={stat.label} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: 0.4 + i * 0.1 }} whileHover={{ scale: 1.05, y: -3 }}
      ...
      <motion.div className="pt-12 md:pt-20 lg:pt-24" id="myResumes"
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
        <motion.div className="dbpx" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}>
          <h2 className="text-primaryBlack flex items-center justify-between text-xl font-bold sm:text-2xl md:text-3xl lg:text-4xl">
            My resumes
            <motion.span className="text-sm font-normal text-gray-400 bg-gray-100 rounded-full px-3 py-1"
