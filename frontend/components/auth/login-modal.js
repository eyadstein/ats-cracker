...
// The actual secret (client_secret) stays on the backend only
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4 z-50">
            <Dialog.Panel className="w-full max-w-[460px] rounded-3xl overflow-hidden shadow-2xl" style={{background: "linear-gradient(135deg, #0f0f0f 0%, #1a1a2e 50%, #16213e 100%)"}}>
                <div className="relative px-10 pt-10 pb-6">
                    ...
                    <Dialog.Title as="h1" className="text-white text-4xl font-black tracking-tight">Welcome<br/>back.</Dialog.Title>
                    <p className="text-gray-500 text-sm mt-2">Sign in to continue building your resume</p>
                </div>
                <div className="px-10 pb-10">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="group">
                            <label className="text-gray-400 text-xs font-semibold tracking-widest uppercase mb-2 block">Email</label>
                            <input type="email" name="email" value={formData.email} onChange={handleChange} disabled={isSubmitting} placeholder="you@example.com" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white placeholder-gray-600 focus:outline-none focus:border-pink-500/50 focus:bg-white/8 transition-all text-sm"/>
                        </div>
                        <div className="group">
                            <label className="text-gray-400 text-xs font-semibold tracking-widest uppercase mb-2 block">Password</label>
                            <div className="relative">
                                <input type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleChange} disabled={isSubmitting} placeholder="••••••••" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white placeholder-gray-600 focus:outline-none focus:border-pink-500/50 focus:bg-white/8 transition-all text-sm pr-16"/>
                                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 text-xs font-semibold tracking-wider uppercase transition-colors">{showPassword ? "Hide" : "Show"}</button>
                            </div>
                        </div>
                        <button type="submit" disabled={isSubmitting} className="w-full py-4 rounded-xl font-black text-white text-sm tracking-wide transition-all mt-2" style={{background: isSubmitting ? "#333" : "linear-gradient(135deg, #e84393, #f97316)", opacity: isSubmitting ? 0.7 : 1}}>{isSubmitting ? "Signing in..." : "Sign In →"}</button>
                    </form>
                    ...
                    <p className="text-center text-gray-600 text-sm mt-6">No account? <button onClick={onChangeModal} className="text-pink-400 font-semibold hover:text-pink-300 transition-colors">Create one</button></p>
                </div>
            </Dialog.Panel>
        </div>
    );
