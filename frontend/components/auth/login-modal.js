...
export default function LoginModal({ onChangeModal, closeModal }) {
    ...
    const googleLogin = () => {
        const clientId = "YOUR_GOOGLE_CLIENT_ID"; // Replace with env var
    ...
    const microsoftLogin = () => {
        const clientId = "YOUR_MICROSOFT_CLIENT_ID"; // Replace with env var
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4 z-50">
            <Dialog.Panel className="w-full max-w-[460px] rounded-3xl overflow-hidden shadow-2xl" style={{background: "linear-gradient(135deg, #0f0f0f 0%, #1a1a2e 50%, #16213e 100%)"}}>
                ...
                <div className="relative px-10 pt-10 pb-6">
                    ...
                    <Dialog.Title as="h1" className="text-white text-4xl font-black tracking-tight">
                        Welcome<br/>back.
                    </Dialog.Title>
                ...
                <div className="px-10 pb-10">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="group">
                            ...
                            <input
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white placeholder-gray-600 focus:outline-none focus:border-pink-500/50 focus:bg-white/8 transition-all text-sm"
                            />
                        </div>
                        ...
                        <div className="group">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    onChange={handleChange} disabled={isSubmitting}
                                    placeholder="••••••••"
                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-white placeholder-gray-600 focus:outline-none focus:border-pink-500/50 focus:bg-white/8 transition-all text-sm pr-16"
                                />
                                <button type="button" onClick={() => setShowPassword(!showPassword)}
                                    className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 text-xs font-semibold tracking-wider uppercase transition-colors">
                        ...
                        <button type="submit" disabled={isSubmitting}
                            className="w-full py-4 rounded-xl font-black text-white text-sm tracking-wide transition-all mt-2"
                            style={{background: isSubmitting ? "#333" : "linear-gradient(135deg, #e84393, #f97316)", opacity: isSubmitting ? 0.7 : 1}}>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <button onClick={googleLogin}
                            className="flex items-center justify-center gap-2 py-3 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all group">
                        ...
                        <button onClick={() => window.location.href = `https://github.com/login/oauth/authorize?client_id=${process.env.NEXT_PUBLIC_GITHUB_CLIENT_ID || "Ov23liOotRANg0xO7IPc"}&scope=user:email`}
                            className="flex items-center justify-center gap-2 py-3 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all group">
                    ...
                    <p className="text-center text-gray-600 text-sm mt-6">
                        No account?{" "}
                        <button onClick={onChangeModal} className="text-pink-400 font-semibold hover:text-pink-300 transition-colors">
                            Create one
                        </button>
                    </p>
                </div>
